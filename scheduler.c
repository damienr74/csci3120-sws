...
static const struct interface _rcb =
		{ sizeof(struct rcb), rcb_new, rcb_delete, };
...
static void sjf_insert(struct rcb *request) {
	if (sched->rcb_count >= sched->capacity) {
		void *p = realloc(sched->rcbs, sizeof(void *) * sched->capacity * 2);
		if (!p) {
...
static const struct interface _sjf_scheduler = { sizeof(struct scheduler),
		sjf_new, sjf_delete, };
...
static const struct interface _Rr_scheduler = { sizeof(struct scheduler),
		rr_new, rr_delete, };
...
static const struct interface _Mlqf_scheduler = { sizeof(struct scheduler),
		mlqf_new, mlqf_delete, };
